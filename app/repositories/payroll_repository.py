#!/usr/bin/env python3
# File: app/repositories/payroll_repository.py
...
class PayrollRepository(BaseRepository):
    def __init__(self, db_adapter: Union[JSONAdapter, SQLAlchemyAdapter]):
        ...
        super().__init__(db_adapter, Payroll)

    async def create(self, schema: CreatePayrollSchema) -> Payroll:
            ...
            async with self.db_adapter.session() as session, session.begin():
                try:
                    ...
                    if "duplicate" in str(e).lower():
                        error_msg = "Payroll already exists!"
                        # raise error_msg
    ...
    async def get_by_id(self, id: UUID) -> Union[Payroll, None]:
        """Get a payroll record from the database by ID"""
        query = (
            select(self.model)
        ...
        async with self.db_adapter.session() as session, session.begin():
            try:
                ...
                raise NotFoundError(detail="Payroll does not exist or ID is invalid")
    ...
    async def get_all(self) -> Sequence[Payroll]:
        """Gets all payroll records from the database"""
        async with self.db_adapter.session() as session, session.begin():
            try:
    ...
    async def get_by_name(self, name: str) -> Optional[Payroll]:
